    ...
    $.fn.watermark = function (text, options) {
        ...
        elems.each(function () {
            ...
            if ($elem.hasClass('hasDatepicker')) {
                var butDate = $elem.next('.ui-datepicker-trigger');
            }
...
.bind('focus.jq_watermark', function () {
    if (!$.watermarker.checkVal($(this).val(), watermark_label)) {
        watermark_label.parent(".watermark_container").addClass('watermark-focused');
        watermark_label.stop().fadeTo(options.animDuration, options.minOpacity);
        }
})
.bind('blur.jq_watermark', function () {
    if (!$.watermarker.checkVal($(this).val(), watermark_label)){
        watermark_label.parent(".watermark_container").removeClass('watermark-focused');
        watermark_label.stop().fadeTo(options.animDuration, 1);
        }
})
.bind('DOMAutoComplete.jq_watermark', function () {
    if (!$.watermarker.checkVal($(this).val(), watermark_label)){
        watermark_label.parent(".watermark_container").removeClass('watermark-focused');
        watermark_label.stop().fadeTo(options.animDuration, 1);
        }
...
.bind('onPropertyChange.jq_watermark', function () {
    if (!$.watermarker.checkVal($(this).val(), watermark_label)){
        watermark_label.parent(".watermark_container").removeClass('watermark-focused');
        watermark_label.stop().fadeTo(options.animDuration, 1);
        }
})
.bind('change.jq_watermark', function () {
    if (!$.watermarker.checkVal($(this).val(), watermark_label)){
        watermark_label.parent(".watermark_container").removeClass('watermark-focused');
        watermark_label.stop().fadeTo(options.animDuration, 1);
        }
