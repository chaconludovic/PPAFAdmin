 ...
 * server-side values and unique client-side strings. In some cases, a {@link ValueEncoder} can be generated
 * automatically from the type of the value parameter. The {@link ValueEncoderSource} service provides an encoder in
 * these situations; it can be overriden by binding the encoder parameter, or extended by contributing a
 * {@link ValueEncoderFactory} into the service's configuration.
 * 
 * @tapestrydoc
 */
...
public class Select extends AbstractField {
    ...
    /**
     * A ValueEncoder used to convert the server-side object provided by the "value" parameter into a unique client-side
     * string (typically an ID) and back. Note: this parameter may be OMITTED if Tapestry is configured to provide a
     * ValueEncoder automatically for the type of property bound to the "value" parameter.
     * 
     ...
     * selected. The value for the blank option is always the empty string, the label may be the blank string; the label
     * is from the blankLabel parameter (and is often also the empty string).
     */
    @Parameter(value = "auto", defaultPrefix = BindingConstants.LITERAL)
    private BlankOption blankOption;

    /**
     * The label to use for the blank option, if rendered. If not specified, the container's message catalog is searched
     * for a key, <code><em>id</em>-blanklabel</code>.
     */
     ...
     * the indicated zone. The component will trigger the event {@link EventConstants#VALUE_CHANGED} to inform its
    ...
    void options(MarkupWriter writer) {
        ...
        // Select unique value if any
        List<OptionModel> options = this.model.getOptions();
        if (isRequired() && options.size() == 1) {
            Object uniqueValue = options.get(0).getValue();
            selectedClientValue = encoder.toClient(uniqueValue);
        } else {
